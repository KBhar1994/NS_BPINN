...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 30 12:44:42 2023
...
from Lang_sampler_2DNS import Lang_sampler

class LangD(object):

    def __init__(self, bayes_nn, n_samples, lr, delta, train_loader1, noise_std):
    ...
    def _optimizers_schedulers(self):
        ...
        for i in range(self.n_samples):
            ...
            optimizer_i = Lang_sampler(self.bayes_nn.nnets[i], parameters, lr=lr, delta=delta) # Lang_sampler(self.bayes_nn.nnets[i], parameters, lr=lr, delta=delta) 
    ...
    def cal_likelihood(self, i, t, x, y, out, dim):
        ...
        inp = torch.cat((t,x,y), 1)
        lambda1 = self.bayes_nn.nnets[i].lambda1
        lambda2 = self.bayes_nn.nnets[i].lambda2
        th1 = self.bayes_nn.nnets[i].th1
        th2 = self.bayes_nn.nnets[i].th2
        ...
        # Psi_hat = torch.reshape(out_hat[:,1], [out_hat.shape[0],1])
        
        # u_hat = torch.autograd.grad(Psi_hat, y, grad_outputs=torch.ones_like(Psi_hat), create_graph = True, retain_graph = True, only_inputs=True)[0]
        # v_hat = - torch.autograd.grad(Psi_hat, x, grad_outputs=torch.ones_like(Psi_hat), create_graph = True, retain_graph = True, only_inputs=True)[0]
        ...
        L1 = lambda1 * u_hat_t + (u_hat*u_hat_x + v_hat*u_hat_y) * lambda1 - P_hat_x - (u_hat_xx + u_hat_yy) * lambda2 + th1
        L2 = lambda1 * v_hat_t + (u_hat*v_hat_x + v_hat*v_hat_y) * lambda1 - P_hat_y - (v_hat_xx + v_hat_yy) * lambda2 + th2
        L3 = u_hat_x + v_hat_y
        ...
        log_like_3 = - 0.5*( ( torch.reshape(out_d[:,2], P_hat.shape) - P_hat)/noise )**2 - 0.5*np.log(2*pi*noise**2)
        log_like_4 = - 0.5*( (L1)/noise )**2 - 0.5*( (L2)/noise )**2 - 0.5*( (L3)/noise )**2
    ...
    def train(self, epoch, epochs, dim):
        ...
        for i in range(self.n_samples):
            ...
            for batch_id, (inp,out) in enumerate(self.train_loader1):
                ...
                log_like_f_all = self.cal_likelihood(i, t, x, y, out, dim)
